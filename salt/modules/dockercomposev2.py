...
.. versionadded:: 2024.0.None

:maintainer: Jamie Murphy
:maturity: new
:depends: docker installed ( newer variant with compose command )
:platform: all
...
This module now requres the `python_on_whales` library instead of the legacy
...
-----------------------
In order to use the module you must have a docker-compose.yml file in existance
(the path where the docker-compose.yml will be read) and an optional
...
__virtualname__ = "dockercomposev2"
...
def __virtual__():
    ...
    return (
        False,
        "The dockercompose execution module not loaded: python_on_whales library not available.",
    )
